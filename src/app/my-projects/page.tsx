...
export default function Page() {
  ...
  const { data: dataGet, isFetching, refetch } = useGetSubmissionQuery(token, { skip: !token, refetchOnMountOrArgChange: true });
          ...
          <Allotment.Pane>
              <Allotment vertical>
                  <Allotment.Pane snap minSize={300}>
                      <select value={language.id} onChange={(e) => {
                  ...
                  <Allotment.Pane maxSize={50}>
                      <button className={`btn ${isFetching || loadingCreate ? 'loading' : ''}`} onClick={() => onClickCompile()}>Submit</button>
                  </Allotment.Pane>
